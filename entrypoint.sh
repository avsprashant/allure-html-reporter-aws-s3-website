...
sh -c "AWS_PROFILE=${AWS_PROFILE} aws s3 ls s3://${AWS_S3_BUCKET}" |  grep "PRE" | sed 's/PRE //' | sed 's/.$//' | sort -nr | while read line;
    do
...
sh -c "AWS_PROFILE=${AWS_PROFILE} aws s3 sync ${SOURCE_DIR:-.} s3://${AWS_S3_BUCKET}/${DEST_DIR} \
...
# Delete history
COUNT=$( sh -c "AWS_PROFILE=${AWS_PROFILE} aws s3 ls s3://${AWS_S3_BUCKET}" | sort -n | grep "PRE" | wc -l )
...
if (( COUNT > INPUT_KEEP_REPORTS )); then
  ...
  sh -c "AWS_PROFILE=${AWS_PROFILE} aws s3 ls s3://${AWS_S3_BUCKET}" |  grep "PRE" | sed 's/PRE //' | sed 's/.$//' | head -n ${NUMBER_OF_FOLDERS_TO_DELETE} | sort -n | while read -r line;
    do
      sh -c "AWS_PROFILE=${AWS_PROFILE} aws s3 rm s3://${AWS_S3_BUCKET}/${line}/ --recursive";
